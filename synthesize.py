...
def synthesize(model, waveglow, melgan, text, sentence, prefix='',
        ...
        speaker_id=None, speaker_embedding=None, prosody_mel_path=None):
    ...
    mel, mel_postnet, log_duration_output, f0_output, energy_output, _, _, mel_len = model(
        ...
        speaker_ids=speaker_ids, speaker_embeddings=speaker_embedding)
...
def mix_voices(speaker_ids, mix_method='mean'):
    # combine the speaker embeddings of multiple speakers
    if mix_method == 'mean':
        # calculates the mean of all embeddings
        total = torch.zeros([1, 128]).to(device)
        for speaker_id in args.speaker_ids:
            speaker_id = torch.tensor(speaker_id).to(torch.int64).to(device)
            total = total + model.module.embed_speakers(speaker_id)
        return total / len(args.speaker_ids)
    elif mix_method == 'half':
        # combines the first half of the first embedding with the
        # second half of the second embedding
        assert len(speaker_ids) == 2, "Half mix only works on 2 voices"
        emb_0 = model.module.embed_speakers(
            torch.tensor(speaker_ids[0]).to(torch.int64).to(device))
        emb_1 = model.module.embed_speakers(
            torch.tensor(speaker_ids[1]).to(torch.int64).to(device))
        return torch.cat([emb_0[:, :64], emb_1[:, 64:]], dim=1)

    elif mix_method == 'subtract':
        # subtract the second from the first
        assert len(speaker_ids) == 2, 'Subtract mix only works on 2 voices'
        emb_0 = model.module.embed_speakers(
            torch.tensor(speaker_ids[0]).to(torch.int64).to(device))
        emb_1 = model.module.embed_speakers(
            torch.tensor(speaker_ids[1]).to(torch.int64).to(device))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mix_voices', type=bool, default=False,
        help='If using a multispeaker model and this is true, the output will'+\
            'be generated by combining the speaker embeddings of all input speakers')
    ...
    if args.mix_voices:
        speaker_embedding = mix_voices(args.speaker_ids, mix_method=args.mix_method)
    ...
    for prosody_mel_path in prosody_paths:
        for i, sentence in enumerate(sentences):
            ...
            if args.speaker_ids is None:
                ...
                if args.mix_voices:
                    synthesize(model, waveglow, melgan, text, sentence,
                        prefix=f'content_mixed_{args.mix_method}'+"-".join(str(l) for l in args.speaker_ids), speaker_id=None,
                        speaker_embedding=speaker_embedding, prosody_mel_path=prosody_mel_path)
                else:
                    for speaker_id in args.speaker_ids:
                        synthesize(model, waveglow, melgan, text, sentence,
