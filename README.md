...
# Multi speaker
The model supports multi speaker training and currently only supports the Talromur corpus. To do multi speaker training, the following hyper parameters have to be specified:
```
multi_speaker = True
speaker_embed_dim = 128 (or something else)
speaker_embed_weight_std = 0.1 (or something else)
```
A multi speaker dataset has to implement the following:
```
dataset.get_speaker_table()
dataset.get_speaker_id_dirname()
```
See `data.talromur.py` and `dataset.py` for more details.
To start training a multi-speaker Talromur model we have to preprocess each voice first, just as if we would for single speaker training. `prepare_talromur.sh` can be used for that. The only things inside `preprocess_dir/multi-speaker` are `train.txt` and `val.txt` which can be generated with `unite_talromur.sh`.
...
# Weights And Biases
Additionally this repository support [Weights and Biases](wandb.ai). To use:
* Install: `pip install wandb`
* Create an account and sign in: `wandb login`
* Set `use_wandb` to True in hparams
