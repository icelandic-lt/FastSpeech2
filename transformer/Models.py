...
class Decoder(nn.Module):
    ...
    def __init__(self,
                 len_max_seq=hp.max_seq_len,
                 d_word_vec=hp.encoder_hidden + hp.speaker_embed_dim + hp.prosody_embed_dim,
                 n_layers=hp.decoder_layer,
                 n_head=hp.decoder_head,
                 d_k=(hp.decoder_hidden + hp.speaker_embed_dim + hp.prosody_embed_dim) // hp.decoder_head,
                 d_v=(hp.decoder_hidden + hp.speaker_embed_dim + hp.prosody_embed_dim) // hp.decoder_head,
                 d_model=hp.decoder_hidden + hp.speaker_embed_dim + hp.prosody_embed_dim,
                 d_inner=hp.fft_conv1d_filter_size,
